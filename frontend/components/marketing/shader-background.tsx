...
const FRAGMENT_SHADER = `
  ...
  vec3 gradient(float d){
      ...
      vec3 colA = mix(vec3(0.05,0.05,0.05),vec3(.4,.4,.6),smoothstep(0.05,0.05,0.05) * d);
...
interface ShaderBackgroundProps {
  className?: string
  /** Hex color for the glass blobs. Defaults to the brand purple. */
  color?: string
}

/**
 * Animated purple "glass" shader used as the hero backdrop on the marketing
 * site. Renders a single full-screen fragment shader via react-three-fiber.
 */
export function ShaderBackground({ className, color = "#a855f7" }: ShaderBackgroundProps) {
